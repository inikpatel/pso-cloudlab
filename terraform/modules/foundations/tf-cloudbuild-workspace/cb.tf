...
locals {
  ...
  environments         = ["shared", "nonproduction", "production"]
  ...
  triggers            = var.trigger_config
  default_triggers_steps = {
    "plan"  = local.triggers
    "apply" = local.triggers
  }
...
resource "google_cloudbuild_trigger" "apply_triggers" {
  ...
  substitutions   = merge(local.default_subst, var.substitutions, { "_WORKSTREAM_PATH" = "${each.value.workstream_path}" })
  service_account = var.cloudbuild_sa[split("-", each.key)[0]].id
  # filename      = local.default_triggers_explicit[each.key]
  filename       = "terraform/modules/foundations/tf-cloudbuild-workspace/templates/cloudbuild-tf-apply.yaml"
  included_files = concat(var.cloudbuild_included_files, [each.value.included_files])
  ignored_files  = var.cloudbuild_ignored_files
}

resource "google_cloudbuild_trigger" "plan_triggers" {
  for_each = var.trigger_config
  ...
  service_account = var.cloudbuild_sa[split("-", each.key)[0]].id
  # filename      = local.default_triggers_explicit[each.key]
  filename       = "terraform/modules/foundations/tf-cloudbuild-workspace/templates/cloudbuild-tf-plan.yaml"
  included_files = concat(var.cloudbuild_included_files, [each.value.included_files])
